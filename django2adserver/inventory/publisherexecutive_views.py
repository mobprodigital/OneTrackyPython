...
from inventory.pubexecutive_serializers import Client_accessSerializer
from inventory.pubexecutive_serializers import Publisher_accessSerializer
from inventory.publisherexecutive_serializers import User_assoc
import json
import secrets
import base64
import hashlib 
...
def bannersStats(request,clientid,campaignid):
    ...
    if tokenStatus:
        ...
        with connection.cursor() as cursor:
            sql = "SELECT b.bannerid,b.description,SUM(s.impressions) AS impressions,SUM(s.clicks) AS clicks,SUM(s.requests) AS requests,SUM(s.total_revenue) AS revenue FROM inventory_banners AS b,inventory_rv_data_summary_ad_hourly AS s WHERE  "+whereStr+" b.bannerid = s.creative_id GROUP BY b.bannerid,b.description"
            cursor.execute(sql)
            field_names = [item[0] for item in cursor.description]
            rawData = cursor.fetchall()
...
def bannersDailyStats(request,clientid,campaignid,bannerid):
    ...
    if tokenStatus:
        ...
        with connection.cursor() as cursor:
            sql = "SELECT SUM(s.impressions) AS impressions,SUM(s.clicks) AS clicks,SUM(s.requests) AS requests,SUM(s.total_revenue) AS revenue,DATE(s.date_time) AS day FROM inventory_banners AS b,inventory_rv_data_summary_ad_hourly AS s WHERE "+whereStr+" b.bannerid = s.creative_id GROUP BY day"
            print(sql)
            cursor.execute(sql)
            field_names = [item[0] for item in cursor.description]
            rawData = cursor.fetchall()
...
def zoneDailyStats(request,pk,id):
    ...
    if tokenStatus:
        ...
        with connection.cursor() as cursor:
            sql = "SELECT SUM(s.impressions) AS impressions, SUM(s.clicks) AS clicks, SUM(s.requests) AS requests, SUM(s.total_revenue) AS revenue, DATE(s.date_time) AS day FROM inventory_rv_data_summary_ad_hourly AS s WHERE " +whereStr+ " GROUP BY day"
            print(sql)
            cursor.execute(sql)
            field_names = [item[0] for item in cursor.description]
            rawData = cursor.fetchall()
...
def zones_list(request,affiliateid):
    tokenempChk         = tokenEmpCheck(request.META['HTTP_AUTHORIZATION'])
    if tokenempChk:
        tokenStatus 		= validateToken(request.META['HTTP_AUTHORIZATION'])
        userId              = getuserIDByToken(request.META['HTTP_AUTHORIZATION'])
        if tokenStatus:	
            if request.method == 'GET':
                ...
                with connection.cursor() as cursor:
                    sql = "SELECT * FROM inventory_zones AS a, inventory_affiliates AS c WHERE " +whereStr+ " a.affiliateid = c.affiliateid"
                    print(sql)
                    cursor.execute(sql)
                    field_names = [item[0] for item in cursor.description]
                    rawData = cursor.fetchall()
...
# def affiliates_list(request,affiliateid):
# 	tokenempChk         = tokenEmpCheck(request.META['HTTP_AUTHORIZATION'])
# 	if tokenempChk:
# 		tokenStatus 		= validateToken(request.META['HTTP_AUTHORIZATION'])
# 		userId              = getuserIDByToken(request.META['HTTP_AUTHORIZATION'])
...
# 						responseObject = {'message': 'Affiliates List', 'data': result, 'status':True}
# 						return JsonResponse(responseObject, safe=False)	
                    
# 					else:
# 						responseObject = {'message': 'No Affiliates Found!', 'data': [], 'status':False}
# 						return JsonResponse(responseObject, safe=False)
                ...
                try:
                    ...
                    if serializer.is_valid():
                        ...
                        if(role==5):
                            userid = (serializer.data).get("user_id")
                            print(affiliateid) 
                            publisheraccessdata = {'affiliateid':affiliateid,'userid': userid}
                        ...
                        # 	userid = (serializer.data).get("user_id")
                        # 	print(affiliateid) 
                        # 	publisheraccessdata = {'parent_id':affiliateid,'child_id': userid}
                        # 	executiveserializer = Publisher_accessSerializer(data=publisheraccessdata)
                        # 	if executiveserializer.is_valid():
                        # 		executiveserializer.save() 		         
