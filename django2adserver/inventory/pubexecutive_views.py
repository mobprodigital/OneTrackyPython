        ...
        if tokenStatus:	
            if request.method == 'GET':
                ...
                if serializer.is_valid():
                    ...
                    f 			= open('delivery/cache/' + zoneCache, 'w+')
...
def users_list(request,affiliateid):
...
def linkedZones(request,campaignid):
    if request.GET.get('period_preset'):
        getDateBYFiletr = getDate(request.GET.get('period_preset'),request)
        print(getDateBYFiletr)
        oStartDate	= getDateBYFiletr['period_start']
    ...
    if tokenStatus:
        campaignidString 		= str(campaignid)
        whereStr 			= 'c.campaignid = '+campaignidString+' AND '
        with connection.cursor() as cursor:
            sql = "SELECT * FROM inventory_zones AS z, inventory_rv_ad_zone_assoc AS s , inventory_banners AS b, inventory_campaigns AS c WHERE " +whereStr+ " c.campaignid = b.campaignid AND z.zoneid = s.zone_id " +getWhereDate(oStartDate, oEndDate)+ " GROUP BY z.zoneid, z.zonename"
            print(sql)
            cursor.execute(sql)
            field_names = [item[0] for item in cursor.description]
            rawData = cursor.fetchall()
...
def ExportPubexecutiveExcel(request):
    ...
    if request.GET['method_name']=='zonestats':
        columns = ['Name', 'Impressions', 'Clicks', 'CTR', 'Revenue', ]        
